"""
Semi-Regulaw Support - Playwright E2E 테스트 (새 UI 기준)
"""
import pytest
...
def test_page_loads(page: Page):
    """메인 페이지 정상 로딩"""
    page.goto(BASE_URL)
    expect(page).to_have_title("Semi-Regulaw Support - 반도체 규제 대응 AI")
...
def test_server_status_online(page: Page):
    """서버 상태 '온라인' 표시 확인"""
    page.goto(BASE_URL)
    expect(page.locator("#serverStatus")).to_contain_text("온라인")


def test_three_step_panels_visible(page: Page):
    """3단계 패널(기업선택·BOM선택·분석조건) 표시 확인"""
    page.goto(BASE_URL)
...
def test_tenant_id_header_sent(page: Page):
    """네트워크 레벨에서 X-Tenant-ID 헤더 전송 확인"""
    page.goto(BASE_URL)
    page.wait_for_timeout(2000)
    ...
    assert "x-tenant-id" in captured, f"X-Tenant-ID 헤더 미전송. 캡처된 헤더: {list(captured.keys())}"
